...
const clampSignedUrlTtl = (value: number): number => Math.min(120, Math.max(60, value));
...
export const signedUrlTtlSeconds = (): number => clampSignedUrlTtl(env.signedUrlExpirationSeconds);
...
export const generateSignedUrl = async (fileKey: string): Promise<string> => {
  ...
  const command = new GetObjectCommand({
    Bucket: env.s3BucketName,
    Key: fileKey,
    ResponseCacheControl: `private, max-age=${expiresIn}, no-transform`,
